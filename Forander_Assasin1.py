...
def PANEL_USE():
    ...
    if num_panel == False:
      ...
      num_panel = True
    console_prompt = input(f"{fg(115)}FORANDER911{fg(114)}@{fg(113)}FORANDER911 {fg(112)}$ {attr(0)}").upper()
    arg_load = console_prompt.split(" ")
    if arg_load[0] == 'HELP FORANDER':
      CLI_COLOR('help')
    elif arg_load[0] == 'BANNER FORANDER':
       CLI_COLOR('banner')
    elif arg_load[0] == 'CLS' or arg_load[0] == 'CLEAR':
       clear_console()
    elif arg_load[0] == 'MENU FORANDER':
       clear_console()
       CLI_COLOR('main_banner')
    elif arg_load[0] == 'METHODS FORANDER':
       if len(arg_load) == 2:
       ...
       if mode_attack == 'INPUT':
        ...
        while True:
         try:
          ...
          try:
             print("EXAMPLE C:\\Users\\FORANDER911\\Desktop\\script.py")
       ...
       if methods_type.upper() == 'L7' or methods_type.upper() == 'LAYER7' or methods_type.upper() == '7':
           ...
           IP = str(input(f'{Fore.LIGHTBLUE_EX}Enter The Target IP/Host ${Fore.RESET}'))
           PORT = int(input(f'{Fore.BLUE}Enter The Target Port ${Fore.RESET}'))
           try:
               s = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
               s.connect((IP,PORT))
               s.send('HI?'.encode())
           ...
           if status_code_tcp == True:
               print(f"{Fore.GREEN}CONNECTION=DONE {Fore.LIGHTGREEN_EX}DATA_RECV={d.decode()}{Fore.RESET}")
           else:
           ...
           try:
               ...
               print(f"{Fore.GREEN}CONNECTION{Fore.WHITE}={Fore.LIGHTCYAN_EX}DONE AKTIF {Fore.YELLOW}( {round(rtt_ms, 2)} ms ){Fore.RESET}")
           except TapError:
               print(f'{Fore.RED}{IP} CONNECTION {Fore.LIGHTRED_EX}TIMEOUT . . .{Fore.RESET}')
    else:
       print(f"{fg(196)}{console_prompt} COMAND TIDAK DI TEMUKAN ! {attr(0)}")
    PANEL_USE()
...
def checker_login():
    ...
    clear_console()
    print(f"{Fore.YELLOW}USER - FORANDER911 {Fore.LIGHTYELLOW_EX}PASSWORD - FORANDER911{Fore.RESET}")
    time.sleep(0.5)
    username = input(f"{Fore.CYAN}MASUKAN USERNAME {Fore.WHITE}${Fore.RESET}")
    time.sleep(0.5)
    password = input(f"{Fore.BLUE}MASUKAN PASSWORD {Fore.WHITE}${Fore.RESET}")
    time.sleep(0.5)
    print(F"{Fore.BLUE}LOGIN TO PANEL FORANDER {Fore.WHITE}({Fore.LIGHTBLUE_EX}TRYING LOGIN WITH {username}@{password}{Fore.WHITE}) . . .{Fore.RESET}")
    time.sleep(int(random.randint(1,3)))
    if login_checker(username,password) == True:
     print(f"{Fore.CYAN}PANEL FORANDER LOADING . . .{Fore.RESET}")
     time.sleep(1)
     clear_console()
     PANEL_USE()
